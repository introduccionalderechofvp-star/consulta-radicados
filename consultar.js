...
async function consultarRadicado(page, numero, alias, directorioSalida) {
  ...
  // Intentar abrir la pestaña/sección de Actuaciones si existe.
  const tabActuaciones = page.getByRole('tab', { name: /actuaciones/i }).first();
  if ((await tabActuaciones.count()) > 0) {
    await tabActuaciones.click().catch(() => {});
    await page.waitForTimeout(1500);
  } else {
    const btnActuaciones = page.getByRole('button', { name: /actuaciones/i }).first();
    if ((await btnActuaciones.count()) > 0) {
      await btnActuaciones.click().catch(() => {});
      await page.waitForTimeout(1500);
    }
  }
  ...
  // Extraer las filas de actuaciones si las encontramos.
  const actuaciones = await page.evaluate(() => {
    ...
    const tablaActuaciones = tablas.find((t) => {
      ...
      return encabezado.includes('actuaci') || encabezado.includes('fecha');
    });
    if (!tablaActuaciones) return [];
    const encabezados = Array.from(tablaActuaciones.querySelectorAll('thead th')).map((th) =>
      th.innerText.trim(),
    );
    const filas = Array.from(tablaActuaciones.querySelectorAll('tbody tr'));
    return filas.map((fila) => {
      const celdas = Array.from(fila.querySelectorAll('td')).map((td) => td.innerText.trim());
      const registro = {};
      celdas.forEach((valor, idx) => {
        const clave = encabezados[idx] || `col_${idx}`;
  ...
  console.log(`  ✔ Actuaciones: ${actuaciones.length} (JSON: ${rutaJson})`);
